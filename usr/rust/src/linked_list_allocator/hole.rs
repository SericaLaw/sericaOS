...
impl HoleList {
    ...
    pub fn allocate_first_fit(&mut self, layout: Layout) -> Result<NonNull<u8>, AllocErr> {
        ...
        // `map` maps a `Result<T, E>` to `Result<U, E>` by applying a function to a
...
fn split_hole(hole: HoleInfo, required_layout: Layout) -> Option<Allocation> {
    ...
    let (aligned_addr, front_padding) = if hole.addr == align_up(hole.addr, required_align) {
        ...
        (
            ...
            Some(HoleInfo {
                addr: hole.addr,
                size: aligned_addr - hole.addr, // front padding
            }),
        )
    };
    ...
    loop {
        ...
        match allocation {
            Some(allocation) => {
                ...
                previous.next = previous.next.as_mut().unwrap() // take out Option<&mut T> next
                    .next.take();   // `take` takes the value out of the option, leaving a [`None`] in its place. 即把当前的next所有权给prev.next
...
fn deallocate(mut hole: &mut Hole, addr: usize, mut size: usize) {
    loop {
        ...
        // `as_ref` converts from `&Option<T>` to `Option<&T>`. 防止被consume
        ...
        match next_hole_info {
            Some(next) if hole_addr + hole.size == addr && addr + size == next.addr => {
                ...
                hole.size += size + next.size; // merge the F and Y blocks to this X block, i.e. ___XXXXXXXXXXXX____
            ...
            Some(next) if addr + size == next.addr => {
                ...
                // before:  ___XXX______YYYYY____Z_    where X is this hole and Y the next hole
                // after:   ___XXX__FFFFYYYYY____Z_    where F is the freed block

                hole.next = hole.next.as_mut().unwrap().next.take(); // remove the Y block
                size += next.size; // free the merged F/Y block in next iteration ___XXX__FFFFFFFFF____Z_
                continue;
            }
