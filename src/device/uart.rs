...
impl Uart {
    ...
    pub fn init(&mut self) {
        ...
        write(self.base_address + COM_LCR, lcr | 1 << 7);

        // Now, base addresses 0 and 1 point to DLL and DLM,
        // respectively. Put the lower 8 bits of the divisor
        // into DLL
        write(self.base_address + COM_DLL, divisor_least);
        write(self.base_address + COM_DLM, divisor_most);
    ...
    pub fn put(&mut self, c: u8) {
        let ptr = self.base_address as *mut u8;
        unsafe {
            write(self.base_address + COM_TX, c);
        }
    }
    ...
    pub fn get(&mut self) -> Option<u8> {
        ...
        unsafe {
            if ptr.add(5).read_volatile() & 1 == 0 {
                ...
                Some(read(self.base_address + COM_RX))
            }
        }
    }
}
